...
export function useMyApartmentsController() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apartmentAdapter.getMy({ size: 100 }),
  });
  ...
  const apartments = useMemo(
    () =>
      statusFilter
        ? allApartments.filter((a) => a.status === statusFilter)
        : allApartments,
  ...
  const { mutate: activate, isPending: isActivating } = useMutation({
    ...
    onError: (error) =>
      showSnackbar(
        getErrorMessage(error, "Could not activate apartment."),
        "error",
      ),
  });
  ...
  const { mutate: deactivate, isPending: isDeactivating } = useMutation({
    ...
    onError: (error) =>
      showSnackbar(
        getErrorMessage(error, "Could not deactivate apartment."),
        "error",
      ),
  });
